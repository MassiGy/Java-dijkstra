...
Cet algorithme correspond à une **implémentation de Dijkstra** utilisant un **tas de Fibonacci** pour la gestion des nœuds et de leurs distances minimales. Cette version de Dijkstra permet une gestion plus performante des **opérations de mise à jour des priorités**, en particulier la **diminution de clé**. 

Effectivement, dans notre algorithme on a utilisé une queue de priorité à la place du tas de Fibonacci ce qui rend l'algorithme moins rapide que celui de GraphStream.


Voici une explication détaillée de l'algorithme :
